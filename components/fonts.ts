import { Pacifico, Whisper, Playwrite_DK_Loopet, Lilita_One, UnifrakturMaguntia, Montserrat, Nanum_Myeongjo, Great_Vibes, Sacramento, Kaushan_Script } from "next/font/google";

export const pacifico = Pacifico({
  weight: "400",
  subsets: ["latin"],
})

export const montserrat = Montserrat({
  weight: "400",
  subsets: ["latin"],
  display: "swap",
})

export const whisper = Whisper({
  weight: "400",
  subsets: ["latin"],
})

export const playwrite = Playwrite_DK_Loopet({
  weight: "400",
})

export const lilita = Lilita_One({
  weight: "400",
  subsets: ["latin"],
})

export const unifraktur = UnifrakturMaguntia({
  weight: "400",
  subsets: ["latin"],
  display: "swap",
})

export const nanum = Nanum_Myeongjo({
  weight: "400",
  subsets: ["latin"],
})

export const greatVibes = Great_Vibes({
  weight: "400",
  subsets: ["latin"],
})

export const sacramento = Sacramento({
  weight: "400",
  subsets: ["latin"],
})

export const kaushan = Kaushan_Script({
  weight: "400",
  subsets: ["latin"],
})